...
;use_SyShell_PTHADD      equ 0   ;Adds a path component to an existing path
;use_SyShell_CHRTST      equ 0   ;Tests if char is available
;use_SyShell_CHRWTC      equ 0   ;Adds or removes a console char watch byte
...
ifdef use_SyShell_CHRTST
    if use_SyShell_CHRTST=1
SyShell_CHRTST0 ld e,0
SyShell_CHRTST
;******************************************************************************
;*** Name           SymShell_CharTest_Command
;*** Input          E  = Channel (0=Standard, 1=Keyboard)
;***                D  = lookahead (0=if available return char but leave it in
;***                     buffer, 1=return char and remove it from buffer)
;*** Output         CF = Error state (0 = ok, 1 = error; A = error code)
;***                - If error status is 0
;***                ZF=1 no char available
;***                C  = char state (0=nothing available, 1=EOF reached,
;***                                 2=char available)
;***                - If error status is 0 and char is available
;***                A  = Next char
;*** Destroyed      F,BC,DE,HL,IX,IY
;*** Description    Reads a char from an input source. The input source can be
;***                the standard channel or the console keyboard. Usually the
...
;***                of the file (EOF) has been reached, the EOF flag will be set.
;******************************************************************************
        ld bc,MSR_SHL_CHRTST*256+MSC_SHL_CHRTST
        call SyShell_DoCommand
        ret c
        ld a,(iy+2)
        ret
...
ifdef use_SyShell_CHRWTC
    if use_SyShell_CHRWTC=1
SyShell_CHRWTC
;******************************************************************************
;*** Name           SymShell_CharWatch_Command
;*** Input          E  = mode (0=stop watching, 1=start watching)
;***                HL = watch byte address
;*** Output         CF = Error state (0 = ok, 1 = error; A = error code)
;*** Destroyed      F,BC,DE,HL,IX,IY
...
;******************************************************************************
        ld a,(App_BnkNum)
        ld d,a
        ld bc,MSR_SHL_CHRWTC*256+MSC_SHL_CHRWTC
        jp SyShell_DoCommand
    endif
endif

ifdef use_SyShell_STRINP
