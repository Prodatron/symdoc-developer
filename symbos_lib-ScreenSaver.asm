...
;  ram area)
;- "App_BegCode" (the first byte of the header/code area)
;is required.
...
SavMai2 ld a,(App_PrcID)      ;*** wait for the next message
        db #dd:ld l,a
        db #dd:ld h,-1
        ld iy,App_MsgBuf
...
SavMai3 ld iy,App_MsgBuf       ;*** handle the received message
        ld a,(App_MsgBuf+0)
        cp MSC_SAV_INIT
...
SavIni  ld a,(App_BegCode+14) ;*** load configuration and test, if valid; if yes, it will overwrite the default config
...
SavEnd  ld a,(App_PrcID)      ;*** quit the screensaver application
        db #dd:ld l,a
        ld a,3
        db #dd:ld h,a
        ld iy,App_MsgBuf
